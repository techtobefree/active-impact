...
| 3 | **Catalog** | Post needs/offers for goods & services, token prices (incl. coupon-style offers), one-step claim that burns the price, tipping |
...
| ~~D9 (Q5)~~ | ~~**Catalog settlement:** claimant requests → poster **accepts** (tokens transfer claimant→poster atomically; quantity decrements; item auto-closes at 0) or **declines**; claimant may cancel. Balance checked at accept, not reserved.~~ | ~~One extra state (`pending`) buys real UX; escrow/reservations deferred.~~ **Superseded by D9b.** |
| D9b | **Catalog settlement is one step and it destroys the tokens.** Claiming an offer *is* the redemption: one transaction burns the snapshot price out of existence, decrements quantity, auto-closes the item at 0, and writes the claim already `redeemed`. There is no pending phase, so there is nothing to accept, decline or cancel. The poster is never paid — they are credited with the deed. All `offer`s are claimable (every offer is priced; 0 = free); `need`s are never claimable — they receive tips. | Two founder decisions, [TOKEN_ONCHAIN.md](../ideas/TOKEN_ONCHAIN.md) **T11** and **T12**. A listing is *binding until withdrawn* (T6), so a poster who could decline would be holding a veto the domain says they do not have — the `pending` state was buying UX for a permission that no longer exists. And the token dies at redemption (T1): paying the poster would recycle it, letting the same token be honoured twice and making outstanding supply mean nothing. Consequence, accepted: a claimant can redeem the same offer repeatedly while quantity lasts — quantity *is* the bound, and the burn is real each time. |
...
│   ├── tokens.py           # ledger read, tip, mint/transfer/burn primitives (the ONLY writers)
│   ├── catalog.py          # items + claims (a claim settles and burns on creation)
