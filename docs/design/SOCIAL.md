...
| **S9c** | **The search box stays on every tab** and searches what that tab shows — projects by title/description/location, Following by person or project. | It first *hid* on Following, because the search only spoke to the projects endpoint. A control that vanishes under your finger is more disorienting than one that changes what it searches, and the founder said so within a day of using it: teaching the feed to search was the smaller price. |
