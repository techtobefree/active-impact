...
├── views/catalog.js        # list (offers|needs tabs), detail, create/edit, redemptions
├── views/wallet.js         # balance, ledger, redemption receipts (both roles), tip form
...
const routes = [
  ...
  [/^#\/scan$/,               views.scanEntry],   // the app bar's scanner (check in from anywhere)
  ...
  [/^#\/me$/,                  views.myProfile],
  [/^#\/u\/([\w-]+)$/,         views.profile],
  [/^#\/u\/(\d+)\/followers$/,  views.followers],   // the list Block lives on
  [/^#\/u\/(\d+)\/following$/,  views.following],
  [/^#\/notifications$/,        views.notifications], // the bell's screen
];
...
| `#/scan` **Scan a code** | The app bar's scanner (CHECKIN_PROOF.md §7.1b). Opens `scanQR()` immediately (detached — the router never queues behind a camera someone is holding up) and routes to whatever it reads: `#/c/…`, `#/s/…`, "that isn't an Active Impact code" + **Scan again**, or — where there is no in-app scanner — a card explaining the native-camera path. Cancel returns to the screen you came from (`scanReturnHash()`). It is not a tab and claims no tab highlight |
...
| `#/catalog/:id` | Detail + role-aware actions. Viewer on offer: **Claim for N 🪙** → a `confirm()` naming the price, because the burn is immediate and irreversible (skipped when free) → toast "Claimed — N 🪙 retired", `refreshMe()` for the app-bar balance. Already redeemed → "show this screen as proof" + the amount retired, and the Claim button **stays** while stock lasts (redeeming twice is two redemptions). `insufficient_balance` reads as "You don't have enough tokens yet". Viewer on need: **Tip** (tip form, `catalog_item_id` attached). Poster: edit/close, **image upload via `imagesStrip` (poster only** — the food example needs a photo**)**, a **Redeemed** section — the retired-token tally + who redeemed, with **no buttons at all** (T6: nothing to approve) |
| `#/wallet` | Balance hero (🪙 big number), **Tip tokens** (recipient **email**, amount, note), ledger list (`direction` arrows, counterparty display name, note, kind chip, local time) — a `burn` row reads "retired — out of circulation" instead of naming a recipient, because it has none. Then two receipt lists: *what I redeemed* + *what people redeemed from me*, both action-free |
| `#/u/:id` **Their page** | Information at the top (initials avatar, display name, bio, joined; stats ⏱ hours · 🪙 earned · 📋 projects; tappable **followers / following** counts), then the actions — **Follow / ✓ Following** and **Tip** — then **their activity feed** (`GET /users/:id/activity`): logged services render as the record card itself, RSVPs and check-ins as a one-line card. Someone they blocked simply sees an empty stream |
| `#/u/:id/followers` · `#/u/:id/following` | The two lists (`person_card[]`). On **my own followers** every row carries **Block / Unblock** — blocking keeps them on the list, because they remain a follower |
| `#/notifications` | The bell's screen: the notifiable activity from people I follow, plus the "tell me when people I follow RSVP or check in" switch. **Opening it marks everything seen** |
...
- Mobile-first single column, `main { max-width: 640px; margin: 0 auto }`,
  sticky top bar, `viewport-fit=cover` + safe-area padding. The top bar carries
  brand · **scan** · **🔔** · balance — scanning is the one drawn (inline-SVG) icon
  in the app, because it is the most-tapped control and no emoji reads as "QR
  code". The bell's unread dot is refreshed on every navigation and by the
  existing 20s version poll — no extra timer.
