...
| `#/` **Projects** | Upcoming project cards (`GET /projects`): cover image on top, then a **details-left / action-right** row — title, 📍 location, 🗓 local time, ⏱ expected duration, checked-in count on the left; the **shared RSVP / check-in / check-out action** (`actionEl`, per-user state now on the card) on the right. The button acts **in place** (stops the card's `<a>` navigation, refreshes just that card via `GET /projects/:id` so the current tab + scroll survive). Client + `q` search. "＋ New project". Tabs: Upcoming · Past · Mine |
