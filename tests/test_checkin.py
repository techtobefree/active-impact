...
A check-in code belongs to an EVENT. Owns the check-in invariants: I11 (a code
resolves only to an OPEN event), I6 (a participation pins the CURRENT waiver
version of the event's project — and a fresh check-in after a waiver edit pins the
NEW version), I3 (one open participation per event/user, re-check-in allowed after
checkout), I12 (the 29/30/90-minute boundaries and the mint cap 600@120 -> 4), and
I1 (a zero-minute checkout mints nothing and leaves balance == ledger sum).
"""
...
def test_resolve_closed_event_is_invalid_code(register):
    owner, _o, _ = register("owner")
    ev = first_event(make_project(owner))
    code = ev["checkin_code"]
    assert owner.post(f"/api/events/{ev['id']}/close").status_code == 200
    # I11: a completed event's code no longer resolves.
...
def test_resolve_shows_my_open_participation(register):
    owner, _o, _ = register("owner")
    code = first_event(make_project(owner))["checkin_code"]
    vol, _v, _ = register("vol")
...
def test_agree_pins_current_waiver(register):
    ...
    detail = make_project(owner)
    ev = first_event(detail)
    code, current_wid = ev["checkin_code"], detail["waiver"]["id"]
...
def test_agree_after_waiver_edit_pins_new_version(register):
    ...
    detail = make_project(owner)
    ev = first_event(detail)
    code, pid, v1_id = ev["checkin_code"], detail["id"], detail["waiver"]["id"]
...
def test_agree_closed_event_404(register):
    owner, _o, _ = register("owner")
    ev = first_event(make_project(owner))
    owner.post(f"/api/events/{ev['id']}/close")
    r = owner.post(f"/api/checkin/{ev['checkin_code']}/agree")
    assert r.status_code == 404 and r.json()["detail"] == "invalid_code"
...
# ---- one open per (event, user), re-check-in after checkout (I3) ------------
...
def test_checkout_by_self(register):
    owner, _o, _ = register("owner")
    code = first_event(make_project(owner, expected_minutes=120))["checkin_code"]
    vol, vu, _ = register("vol")
